...
class LRUReplPolicy_DPG_d : public ReplPolicy {
    protected:
    ...
    public:
        explicit LRUReplPolicy_DPG_d(uint32_t _numLines) : timestamp(1), numLines(_numLines) {
    ...
    private:
        inline uint64_t score(uint32_t id) { //higher is least evictable
            ...
            // (2) sharers, and
            // (3) timestamp
            //check the ID here to see if its working
            //info("id : %d and numines %d", id, numLines/2)
 ...
 */
template <bool sharersAware>
class LRUReplPolicy_DPG_t : public ReplPolicy {
    protected:
        uint64_t timestamp; // incremented on each access
        uint64_t* array;
        uint32_t numLines;
    ...
    public:
        explicit LRUReplPolicy_DPG_t(uint32_t _numLines) : timestamp(1), numLines(_numLines) {
            array = gm_calloc<uint64_t>(numLines);
        }
    ...
    private:
        inline uint64_t score(uint32_t id) { //higher is least evictable
            ...
            //check the ID here to see if its working
            //info("id : %d and numines %d", id, numLines/2)
            return (sharersAware? cc->numSharers(id) : 0)*timestamp + array[id]*cc->isValid(id + numLines);
        }
};
